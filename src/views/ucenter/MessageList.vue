          ...
          <template #label>
            <div class="tab-item">
             ...
             <span class="count-tag"  v-if="messageCountInfo.reply>0">
               {{messageCountInfo.reply>99?99:messageCountInfo.reply}}</span>
            </div>
          </template>
        </el-tab-pane>
          ...
          <template #label>
            <div class="tab-item">
              ...
              <span class="count-tag"  v-if="messageCountInfo.likePost>0">
               {{messageCountInfo.rlikePost>99?99:messageCountInfo.likePost}}</span>
            </div>
          </template>
        </el-tab-pane>
          ...
          <template #label>
            <div class="tab-item">
             ...
             <span class="count-tag"  v-if="messageCountInfo.downloadAttachment>0">
               {{messageCountInfo.downloadAttachment>99?99:messageCountInfo.downloadAttachment}}</span>
            </div>
          </template>
        </el-tab-pane>
        ...
        <template #default="{data}">
          ...
          <div class="message-item" v-if="data.messageType==0">
            <div class="message-content">
              ...
              <span class="create-item">{{data.createTime}}</span>
            </div>
          </div>
          <!-- 回复我的 -->
          <div class="message-item" v-if="data.messageType==1">
            ...
            <div class="message-content">
              ...
              对我的文章【<router-link class="a-link" :to="`/user/${data.articleId}`">{{data.articleTitle}}</router-link>】发表了评论
              <span class="create-item">{{data.createTime}}</span>
            </div>
...
const loadMessage = async () => {
  loading.value = true;
  let params = {
    pageNo: messageListInfo.value.pageNo?messageListInfo.value.pageNo:1,
...
watch(
  ...
  (newVal, oldVal) => {
    if(newVal){
    activeTabName.value=newVal.type
    tabChange(newVal.type)
    loadMessage()
    }
  }, 
  { immediate: true, deep: true }
